...
<template>
	<div
		v-if="Array.isArray(getPosts) && getPosts.length > 0"
		...
		<div class="row">
			...
			>
				<div class="card mb-4 post">
					<img
						class="card-img-top"
						:src="item.image ? apiDomain + item.image : defaultImage"
						:alt="item.title"
					/>
					<div
						v-if="Array.isArray(item.postCategories) && item.postCategories.length > 0"
						class="categories"
					>
